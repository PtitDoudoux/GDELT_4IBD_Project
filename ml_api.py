...
@app.route('/predict')
def predict(date, actor1, actor2):
    """
    Compute a prediction for 15 days based on a date and the actors
    :param date: The date of the event to predict
    :param actor1: The Actor who execute the action
    :param actor2: The actor who receive the action
    :return: The result as a JSON
    """
    predicted = list()
    to_predict = np.zeros((16, 21), dtype=np.float32)
    headers_cols = HEADER.split(',')
    input_cols = {headers_cols[i]: i for i in range(len(headers_cols[:21]))}
    output_cols = {headers_cols[i + 21]: i for i in range(len(headers_cols[21:]))}
    ddays = utils.list_of_dates(date, '%Y%m%d', 16)
    for i in range(16):
        to_predict[i][0] = utils.normalize_date(ddays[i], '20080101', '20190101', '%Y%m%d')
        to_predict[i][input_cols[actor1.lower()]] = 1.0
        to_predict[i][input_cols[actor2.lower()]] = 1.0
    predictions = list(MODEL.predict(to_predict, batch_size=1))
    for i in range(16):
        predicted.append({ddays[i]: output_cols[np.argmax(predictions[i])]})
    return predicted
